...
| 能力 | 说明 |
|---|---|
| 房间密码 | 设在房间上，重开会议依然有效，存的是 scrypt 哈希 |
...
未验证（需要真实环境）：真实多机 P2P 打通、Tailscale 跨地域连通质量、真实 ASR 的中文识别准确率、手机端实机表现、**虚拟背景（需要浏览器和 GPU）**、**Electron 桌面壳（需要图形界面）**。后两项只保证代码能构建、语法和 API 用法正确。
